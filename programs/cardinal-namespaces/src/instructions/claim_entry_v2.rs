...
pub struct ClaimEntryV2Ix {
    pub duration: Option<i64>,
}

#[derive(Accounts)]
#[instruction(ix: ClaimEntryV2Ix)]
pub struct ClaimEntryV2Ctx<'info> {
    #[account(mut)]
    ...
    #[account(mut, constraint = mint.key() == entry.mint @ ErrorCode::InvalidEntryMint)]
...
pub fn handler<'key, 'accounts, 'remaining, 'info>(ctx: Context<'key, 'accounts, 'remaining, 'info, ClaimEntryV2Ctx<'info>>, ix: ClaimEntryV2Ix) -> Result<()> {
    let remaining_accs = &mut ctx.remaining_accounts.iter();
    let entry = &mut ctx.accounts.entry;
    entry.data = Some(ctx.accounts.recipient.key());
